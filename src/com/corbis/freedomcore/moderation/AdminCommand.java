	   ...
	   @Override
	    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		   if(sender instanceof Player) {
			   if(cmd.getName().equalsIgnoreCase("admin")) {
				   ...
				   if(args.length == 1) {
					   if(p.hasPermission("freedom.admin")) {
						   p.getServer().dispatchCommand(p.getServer().getConsoleSender(), "manuaddp " + t.getName() + " freedom.admin");
			   ...
			   if(cmd.getName().equalsIgnoreCase("admin")) {
				   Player t = Bukkit.getPlayer(args[0]);
				   t.getServer().dispatchCommand(t.getServer().getConsoleSender(), "manuaddp " + t.getName() + " freedom.admin");
			   }
		   }
		return false;
	   }
